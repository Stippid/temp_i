		...
		<div class="col-md-12" align="center">
			<div class="card">
				<div class="card-header">
					<h5>ALL INDIA HOLDING REPORT</h5>
					...
					<div class="col-md-12">
						<div class="col-md-6">
							...
							<div class="row form-group">
								...
								<div class="col-md-8">
									<select path="a_type" id="a_type" class="form-control">
									<option value="0" >--Select--</option>
...
<%-- 										</c:forEach> --%>
						...
						<div class="col-md-6">
							<div class="row form-group">
								...
								<div class="col-md-8">
								<select path="b_head" id="b_head" class="form-control" onchange="fn_B_Head()">
											<option value="0">--Select--</option>
...
<%-- 									<form:input type="text" id="b_head" path="b_head" class="form-control autocomplete" autocomplete="off"></form:input> --%>
						...
						<div class="col-md-6">
							<div class="row form-group">
								...
								<div class="col-md-8">
								<select path="b_code" id="b_code" class="form-control" >
											<option value="0">--Select--</option>
				...
				<table id="getSearch_All_India_Holding"
					...
					<thead>
						<tr>
							...
							<th id ="11">Servicable State</th>
							<th id ="12">UN-Servicable State</th>
...
<c:url value="Download_All_India_Holding_Details" var="dwonloadUrl" />
...
<c:url value="SearchAll_Holding" var="SearchAll_Holding" />
<form:form action="${SearchAll_Holding}" method="post" id="searchForm" name="searchForm" modelAttribute="id7">
...
<c:url value="Excel_All_India_Holding_Details" var="excelUrl" />
<form:form action="${excelUrl}" method="post" id="ExcelForm" name="ExcelForm" modelAttribute="cont_comd_ex">
	 <input type="hidden" name="cont_comd_ex" id="cont_comd_ex"  value="0">
	   ...
	   <input type="hidden" name="sus_no_ex" id="sus_no_ex">
	    <input type="hidden" name="parent_arm_ex" id="parent_arm_ex">
	   <input type="hidden" name="cadre_ex" id="cadre_ex">
...
function data(getSearch_All_India_Holding){
	...
	$.post("getindiaholding?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,asset_type:asset_type,
		a_type:a_type,b_head:b_head,b_code:b_code,a_type:a_type,s_state:s_state,
		},function(j) {
			
		for (var i = 0; i < j.length; i++) {
			
			jsondata.push([j[i].command,j[i].corps,j[i].div,j[i].bde,j[i].unit_name,j[i].sus_no,j[i].asset_cat,j[i].assets_name,j[i].make_name,j[i].model_name,j[i].model_number,j[i].machine_number,j[i].b_head,j[i].budget_code,j[i].service_state,j[i].unservice_state,
			]);
	...
	$.post("getindiaholdingCount?"+key+"="+value,{Search:Search,cont_comd:cont_comd,cont_corps:cont_corps,cont_div:cont_div,cont_bde:cont_bde,unit_name:unit_name,sus_no:sus_no,asset_type:asset_type,b_head:b_head,b_code:b_code,a_type:a_type,s_state:s_state,
		},function(j) {
		FilteredRecords = j;
	});
}
...
// 		    	getCONTBde_ready();
	   	...
	   	$('select#cont_comd').change(function() {
		   	...
		   	$('select#cont_corps').change(function() {
	   	   	...
	   	   	if(fcode == "0"){
		   		 ...
		   		 $("select#cont_div").html(select);
	   	   		//$("select#cont_bde").html(select); 
		   	   		getCONTDiv_onchange(fcode);
		       		fcode += "000";	
		       		getCONTBde_Corpsonchange(fcode);
		   	}
		   	});
	...
	function getCONTCorps(fcode){
		var fcode1 = fcode;
			$.post("getCorpsDetailsList_onchange?"+key+"="+value,{fcode:fcode1}, function(j) {
				if(j != ""){
	 ...
	 function getCONTDiv(fcode){
		var fcode1 = fcode;
		   	$.post("getDivDetailsList_onchange?"+key+"="+value,{fcode:fcode1}, function(j) {
		   		if(j != ""){
	 		   	var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
	 ...
	 function getCONTDiv_onchange(fcode){
		...
		function getCONTBde(fcode){
			var fcode1 = fcode;
			$.post("getBdeDetailsList_onchange?"+key+"="+value,{fcode:fcode1}, function(j) {
				if(j != ""){
					var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
		...
		function getCONTBde_Corpsonchange(fcode){
			var fcode1 = fcode[0] + fcode[1] + fcode[2];
			$.post("getBdeDetailsList_onchange?"+key+"="+value,{fcode:fcode1}, function(j) {
				if(j != ""){
					var length = j.length-1;
		...
		function getCONTBde_onchange(fcode){
		...
		function getCONTCorps_ready(){
			var level = 'Corps';
				$.post("getCorpsDetailsList_ready?"+key+"="+value,{level:level}, function(j) {
					if(j != ""){
						var length = j.length-1;
						var enc = j[length][0].substring(0,16);
						var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
		 ...
		 function getCONTDiv_ready(){
			 var level ='Division';
				   	$.post("getDivDetailsList_ready?"+key+"="+value,{level:level}, function(j) {
				   		if(j != ""){
			 		   	var length = j.length-1;
						var enc = j[length][0].substring(0,16);
						var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
		...
		function getCONTBde_ready(){
			var level ='Brigade';
			$.post("getBdeDetailsList_ready?"+key+"="+value,{level:level}, function(j) {
				if(j != ""){
					var length = j.length-1;
				var enc = j[length][0].substring(0,16);
				var options = '<option value="' + "0" + '">'+ "--Select--" + '</option>';
