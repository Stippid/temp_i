...
<script nonce="${cspNonce}">
	...
	function data(tableName) {
		...
		$.post("getOffice_mstr_Other_ReportDataList?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				var data = [ j[i].office, j[i].chekboxaction ];
				jsondata.push(data);
			}
		...
		$.post("getOffice_mstr_other_TotalCount?" + key + "=" + value, {
			Search : Search,
			office : office,
			status : status
		}, function(j) {
	...
	function setApproveStatus() {
		...
		if (a == "") {
			...
			$.post("Approve_OfficeotherData?" + key + "=" + value, {
				a : a,
				status : "1"
			}).done(function(j) {
				alert(j);
	...
	function setDeleteStatus() {
		...
		if (a == "") {
			...
			$.post("Delete_OfficeotherData?" + key + "=" + value, {
				a : a
			}).done(function(j) {
				alert(j);
				location.reload();
