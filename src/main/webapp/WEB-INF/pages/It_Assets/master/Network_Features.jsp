...
<form:form name="Formname" id="Formid"
	action="Network_Features_Action?${_csrf.parameterName}=${_csrf.token}"
	method="POST" modelAttribute="Network_Features_CMD">
	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>NETWORK SECURITY FEATURES MASTER</h5>
			</div>
			...
			<div class="card-footer" align="center" class="form-control">
				<a href="NETWORK_FEATURES_Url" class="btn btn-success btn-sm">Clear</a>
...
<c:url value="EditNetwork_Features_Url" var="updateUrl" />
...
<c:url value="Delete_Network_Features_Url" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm"
	name="deleteForm" modelAttribute="deleteid">
	<input type="hidden" name="deleteid" id="deleteid" />
</form:form>

<c:url value="Networkmastereport" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	name="searchForm" modelAttribute="comd1">
	<input type="hidden" name="typeReport1" id="typeReport1" value="0" />
	<input type="hidden" name="network_features1" id="network_features1"
		value="0" />
	...
	<div class="card-body">
		<div class="" id="divPrint">
			 ...
			 <button class="download-excel-btn" id="btnExport" title="EXPORT TO EXCEL"><i class="fa fa-file-excel-o"></i> Download Excel</button> 
				<div class="col-md-12">
					<table id="Network_Features_mstr_reporttbl"
						class="display">
						<thead>
							<tr>
								<th>Network Security Features</th>
								<th>Action</th>
							</tr>
						</thead>
					</table>
...
<script nonce="${cspNonce}">
	...
	$(document).ready(function() {
		...
		mockjax1('Network_Features_mstr_reporttbl');
		table = dataTable('Network_Features_mstr_reporttbl');
		try {
	...
	function data(tableName) {
		jsondata = [];
		var table = $('#Network_Features_mstr_reporttbl').DataTable();
		var info = table.page.info();
		var network_feature = $("#network_features").val();
		...
		$.post("getNetwork_Features_mstrReportDataList?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([ j[i].network_features, j[i].id ]);
			}
		});
		$.post("getNetwork_Features_mstrTotalCount?" + key + "=" + value, {
			Search : Search,
			network_feature : network_feature
		}, function(j) {
			FilteredRecords = j;
	...
	function getExcel() {
		...
		var network_features = $("#network_features").val();
		$("#network_features1").val(network_features);
