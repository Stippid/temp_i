...
<form:form name="Formname" id="Formid"
	action="ups_type_mstrAction?${_csrf.parameterName}=${_csrf.token}"
	method="POST" modelAttribute="ups_type_mstrCMD">
	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>UPS TYPE MASTER</h5>
			</div>
						...
						<div class='row form-group'>
							...
							<div class='col-12 col-md-8'>
								<form:input type="text" id="ups_type" path="ups_type"
									class="form-control" autocomplete='off' maxlength="50"></form:input>
							</div>
						</div>
					</div>
				</div>
			...
			<div class='card-footer' align='center'>
				<a href=Ups_type_mstr_Url class="btn btn-success btn-sm">Clear</a> <input
...
<c:url value="EditUps_type_mstrUrl" var="updateUrl" />
...
<c:url value="deleteups_type_mstrUrl" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm"
	name="deleteForm" modelAttribute="deleteid">
	<input type="hidden" name="deleteid" id="deleteid" />
</form:form>

<c:url value="UPSmastereport" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	name="searchForm" modelAttribute="comd1">
	<input type="hidden" name="typeReport1" id="typeReport1" value="0" />
	<input type="hidden" name="ups_type1" id="ups_type1" value="0" />
		...
		<div class="card-body">
			<div class="" id="divPrint">
				<div class="container" align="center">
					...
					<div class="col-md-12">
						<table id="ups_type_mstr_reporttbl"
							class="display">
							<thead>
								<tr>
									<th>Type Of UPS</th>
...
<script nonce="${cspNonce}">
	...
	$(document).ready(function() {
		...
		mockjax1('ups_type_mstr_reporttbl');
		table = dataTable('ups_type_mstr_reporttbl');
		try {
	...
	function data(tableName) {
		jsondata = [];
		var table = $('#ups_type_mstr_reporttbl').DataTable();
		var info = table.page.info();
		var ups_type = $("#ups_type").val();
		...
		$.post("getUps_type_mstrReportDataList?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([ j[i].ups_type, j[i].id ]);
			}
		});
		$.post("getUps_type_mstrTotalCount?" + key + "=" + value, {
			Search : Search,
			ups_type : ups_type
		}, function(j) {
			FilteredRecords = j;
