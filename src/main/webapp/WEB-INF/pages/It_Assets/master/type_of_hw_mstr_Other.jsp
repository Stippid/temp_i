	...
	<div class="container" align="center">
		<div class="card">
					...
					<div class='col-md-6'>
						<div class='row form-group'>
							...
							<div class='col-12 col-md-8'>
								...
								<div id="toh" class='col-md-12 f_11 color_red'></div>
							</div>
						</div>
					</div>
				</div>
			...
			<div class='card-footer' align='center'>
				<a href=Type_of_hw_Other_mstr_Url class="btn btn-success btn-sm">Clear</a>
		...
		<div class="card-body">
			<div class="watermarked" data-watermark="" id="divwatermark">
				<span id="ip"></span> <input type="hidden" id="CheckVal"
					...
					name='tregn' onclick='setselectall();'>Select all [<span
					id="tregn">0</span><span id='nTotRow1'>/</span><span id="tregnn">
						${list.size()}</span>]</b>
				<table id="type_of_hw_mstr_reporttbl"
					class="display">
					<thead>
						<tr>
...
<script nonce="${cspNonce}">
	...
	function data(tableName) {
		...
		$.post("getType_of_hw_other_mstrReportDataList?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				var data = [ j[i].assets_name, j[i].type_of_hw,
						j[i].chekboxaction ];
				jsondata.push(data);
			}
		...
		$.post("getType_of_hw_Other_mstrTotalCount?" + key + "=" + value, {
			Search : Search,
			peripheral_type : peripheral_type,
			type_of_hw : type_of_hw,
			status : status
	...
	function setApproveStatus() {
		...
		if (a == "") {
			...
			$.post("Approve_type_of_hw_Data?" + key + "=" + value, {
				a : a,
				status : "1"
			}).done(function(j) {
				alert(j);
	...
	function setDeleteStatus() {
		...
		if (a == "") {
			...
			$.post("Delete_type_of_hw_Data?" + key + "=" + value, {
				a : a
			}).done(function(j) {
				alert(j);
				location.reload();
