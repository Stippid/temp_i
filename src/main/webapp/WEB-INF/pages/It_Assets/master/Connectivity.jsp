...
<form:form name="Formname" id="Formid"
	action="Connectivity_Action?${_csrf.parameterName}=${_csrf.token}"
	method="POST" modelAttribute="Connectivity_CMD">
	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>CONNECTIVITY TECHNOLOGY MASTER</h5>
			</div>
			...
			<div class="card-footer" align="center" class="form-control">
				<a href=CONNECTIVITY_Url class="btn btn-success btn-sm">Clear</a> <input
...
<c:url value="EditConnectivity_Url" var="updateUrl" />
...
<c:url value="Delete_Connectivity_Url" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm"
	name="deleteForm" modelAttribute="deleteid">
	<input type="hidden" name="deleteid" id="deleteid" />
</form:form>
<c:url value="Connectivitymastereport" var="searchUrl" />
<form:form action="${searchUrl}" method="post" id="searchForm"
	name="searchForm" modelAttribute="comd1">
	<input type="hidden" name="typeReport1" id="typeReport1" value="0" />
	<input type="hidden" name="connectivity_type1" id="connectivity_type1"
		value="0" />
		...
		<div class="card-body">
			<div class="" id="divPrint">
				...
				<div class="watermarked" data-watermark="" id="divwatermark">
					<div class="col-md-12">
						<table id="Connectivity_mstr_reporttbl" class="display">
							<thead>
								<tr>
...
<script nonce="${cspNonce}">
	document.addEventListener('DOMContentLoaded', function() {
		...
		document.addEventListener('click', function(event) {
			...
			if (target.classList.contains('action_update')) {
				const encryptedPk = target.getAttribute('data-encrypted-pk');
				if (confirm('Are you sure you want to Update?')) {
					editData(encryptedPk); // Call your update function here
				}
			} else if (target.classList.contains('action_delete')) {
				const encryptedPk = target.getAttribute('data-encrypted-pk');
				if (confirm('Are you sure you want to Delete?')) {
					deleteData(encryptedPk); // Call your delete function here
				}
			}
		});
	...
	function data(tableName) {
		...
		$.post("getConnectivity_mstrReportDataList?" + key + "=" + value, {
			startPage : startPage,
			...
			for (var i = 0; i < j.length; i++) {
				jsondata.push([ j[i].connectivity_type, j[i].id ]);
			}
		});
		$.post("getConnectivity_mstrTotalCount?" + key + "=" + value, {
			Search : Search,
			connectivity_type : connectivity_type
		}, function(j) {
			FilteredRecords = j;
	...
	function getExcel() {
		...
		$("#connectivity_type1").val(connectivity_type);
