...
<form:form name="Formname" id="Formid"
	action="MAINTENANCEAction?${_csrf.parameterName}=${_csrf.token}"
	method="POST" modelAttribute="MAINTENANCECMD">
	<div class="container" align="center">
		<div class="card">
			<div class="card-header">
				<h5>MAINTENANCE AGENCY MASTER</h5>
			</div>
					...
					<div class='row form-group'>
						<div class='col col-md-6'>
							<strong class="color_red">*</strong> <label for="text-input"
								class="form-control-label">Maintenance Agency NAME</label>
						</div>
...
<c:url value="deleteMAINTENANCEUrl" var="deleteUrl" />
<form:form action="${deleteUrl}" method="post" id="deleteForm"
	name="deleteForm" modelAttribute="deleteid">
	<input type="hidden" name="deleteid" id="deleteid" />
</form:form>
<c:url value="EditMAINTENANCEUrl" var="updateUrl" />
...
$(document).ready(function () {
	mockjax1('MAINTENANCE_reporttbl');
	table = dataTable('MAINTENANCE_reporttbl');
...
function data(tableName){
	jsondata = [];
	var table = $('#MAINTENANCE_reporttbl').DataTable();
	...
	$.post("getMaintenanceReportDataList?"+key+"="+value,{startPage:startPage,pageLength:pageLength,Search:Search,orderColunm:orderColunm,orderType:orderType,maintenance:maintenance},function(j) {
		for (var i = 0; i < j.length; i++) {
			jsondata.push([j[i].maintenance,j[i].id]);
		}
	});
	$.post("getMaintenanceTotalCount?"+key+"="+value,{Search:Search,maintenance:maintenance},function(j) {
		FilteredRecords = j;
	});
}
document.addEventListener('DOMContentLoaded', function() {
...
if($("#maintenance").val().trim() == "")
 {
	  alert("Please Enter Maintenance Name");
	  $("#maintenance").focus();
	  return false;
 } 
return true; 
